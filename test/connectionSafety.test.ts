...
describe('PairingManager', () => {
  it('requires the exact code and supports revocation', () => {
    ...
    expect(() =>
      manager.confirm(session.id, 'WRONG', 'phone', 200)
    ).toThrow();

    manager.confirm(session.id, session.code, 'phone', 200);

    expect(
      manager.isAuthorized(session.id, 'phone', 300)
    ).toBe(true);

    manager.revoke(session.id);

    expect(
      manager.isAuthorized(session.id, 'phone', 301)
    ).toBe(false);
  });
  ...
  it('expires sessions', () => {
    ...
    expect(() =>
      manager.confirm(session.id, session.code, 'phone', 1101)
    ).toThrow('expired');
  });
});
